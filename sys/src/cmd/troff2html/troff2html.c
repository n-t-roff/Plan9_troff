...
header(char *s)
{
	Bprint(&(bout.Biobufhdr), "<head>\n");
	Bprint(&(bout.Biobufhdr), "<title>%s</title>\n", s);
	Bprint(&(bout.Biobufhdr), "<meta content=\"text/html; charset=utf-8\" http-equiv=Content-Type>\n");
	Bprint(&(bout.Biobufhdr), "</head>\n");
	Bprint(&(bout.Biobufhdr), "<body bgcolor=#ffffff>\n");
}
...
trailer(void)
{
	...
	t0 = time(nil);
	t = localtime(&t0);
	Bprint(&(bout.Biobufhdr), TABLE "<tr height=20><td></table>\n");
	Bprint(&(bout.Biobufhdr), "<font size=-1><a href=\"http://www.lucent.com/copyright.html\">\n");
	Bprint(&(bout.Biobufhdr), "Copyright</A> &#169; %d Alcatel-Lucent.  All rights reserved.</font>\n", t->tm_year+1900);
#endif
	Bprint(&(bout.Biobufhdr), "</body></html>\n");
}
