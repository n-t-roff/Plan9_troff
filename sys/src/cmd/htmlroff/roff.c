...
Brune(Rune r)
{
	if(r == '&')
		...
		Bprint(&bout.Biobufhdr, "%C", r);
	else
		Bprint(&bout.Biobufhdr, "%S", rune2html(r));
}
...
outrune(Rune r)
{
	...
	case Uminus:
		...
		break;
	case '\'':
		Bprint(&bout.Biobufhdr, "&rsquo;");
		return;
	case '`':
		Bprint(&bout.Biobufhdr, "&lsquo;");
		return;
	case Uamp:
		Bputrune(&bout.Biobufhdr, '&');
		return;
	case Ult:
		Bputrune(&bout.Biobufhdr, '<');
		return;
